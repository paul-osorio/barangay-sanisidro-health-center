        ...
        function checkUnreads() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data > 0) {
                        $('#messnot').show();
                        document.title = "(1) Inbox | Patient";

                    } else {
                        $('#messnot').hide();
                        document.title = "Inbox | Patient";
                    }
                }
            })
        }
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        for (var count = 0; count < data.length; count++) {
                            ...
                            output += '<p style="font-size: 14px" class="' + ((data[count].chatstatus === "unread") ? 'text-dark fw-bold' : 'text-read') + ' mb-0">' + data[count].firstname + ' ' + data[count].lastname + '</p></div>';
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        for (var count = 0; count < data.length; count++) {
                            ...
                            output += '<a href="' + base_url + '/DoctorChat/setReceiverSession/' + data[count].user_id + '" data-forclass="' + 'thisactive_' + data[count].user_id + '" class="user_chat_list" data-receiver_profile="' + ((hasProfile) ? '<?= base_url('uploads/Doctor/') ?>' + data[count].profile : '<?= base_url('assets/images/profile.png') ?>') + '" data-receiver_name="' + data[count].firstname + ' ' + data[count].lastname + '" data-receiver_id="' + data[count].user_id + '">'
                            output += '<div class="userchatlist"><div class="border-bottom chatuserbox " id="chitchat" >';
                            output += '<div class="row gx-0 py-2">';
                            output += '<div class="col-lg-3 d-flex justify-content-center align-items-center">';
