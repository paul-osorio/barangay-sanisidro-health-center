        ...
        <div class="col-lg-12">
            ...
            <div class="card card-body border-0" style="border-radius: 10px; box-shadow: 0 4px 4px rgba(0,0,0,0.10)">
                <div class="d-flex justify-content-between">
                    <label class="lblback fs-5" onclick="location.href='<?= base_url('P/HealthService') ?>'"><i class="fa-solid fa-arrow-left"></i></label>
                    <label class="fs-5" style="color: #005AE8"><?= pediaIcon(25, 25) ?>FOR PEDIA</label>
                    <label for=""></label>
                </div>
                <div class="row gy-3 mt-3">
                    ...
                    foreach ($profile as $user) {
                        ...
                        $yeardate = $diff->format('%y');
                        $monthdate = $diff->format('%m');
                        // $bday = (($yeardate > 0) ? $yeardate . ' year/s old' :  ($monthdate > 1) ?. ' month old');

                        if ($yeardate > 0) {
                            if ($yeardate > 1) {
                                $bday = $yeardate . ' years old';
                            } else {
                                $bday = $yeardate . ' year old';
                            }
                        } else {
                            if ($monthdate > 1) {
                                $bday = $monthdate . ' months old';
                            } elseif ($monthdate === 1) {
                                $bday = $monthdate . ' month old';
                            } else {
                                $bday = 'less than a month';
                            }
                        }
                    ...
                    ?>
                        <div class="col-md-6 col-lg-4">
                            <div class="position-relative">
                                <div class="card card-body profcard " onclick="location.href='<?= base_url('P/ForPedia2/' . $user['ID']) ?>'">
                                    <div class="d-flex justify-content-between">
                                        <div class="">
                                            <div class="">
                                                <label for="" style="cursor: pointer" class="d-block"><?= $user['Firstname'] . ' ' . $user['Middlename'] . ' ' . $user['Lastname'] ?></label>
                                                <label for="" style="cursor: pointer" class="d-block">Age: <?= $bday  ?></label>
                ...
                <div class="mt-2">
                    ...
                    $res = $this->db->query('SELECT * FROM otheraccount WHERE PatientID=' . $uid . ' AND Type="pedia"');

                    ?>

                    <button class="newprof" data-bs-toggle="modal" data-bs-target="#exampleModal" <?= ($res->num_rows() >= 5) ? 'disabled' : '' ?>><i class="fas fa-plus plusrot"></i> Add New Profile</button>
                </div>
            </div>
        </div>
    </div>
            ...
            <form action="<?= base_url('PatientAuth/addprofile') ?>" method="POST" id="addprofileform">
                <div class="modal-body">
                    <div class="row gx-2 gy-3">
                        <input type="text" value="pedia" name="type" hidden>
                        <div class="col-lg-4">
                        ...
                        <div class="col-lg-6">
                            <input type="text" placeholder="Date of Birth" id="pbday" name="birthday" placeholder="Date of Birth" class="inpt">
    ...
    <div class="modal-dialog">
        <div class="modal-content border-0" style="background-color: rgba(245,245,245)">
            ...
            <form action="<?= base_url('PatientAuth/updateprofile') ?>" method="POST" id="updateformprofile">
                <div class="modal-body">
                    <div class="row gx-2 gy-3">
                        <input type="text" name="type" value="pedia" hidden>
                        <input type="text" name="id" id="mmyd" hidden>
                        <div class="col-lg-4">
                            <input type="text" onkeydown="return alphaOnly(event)" name="firstname" id="ffname" placeholder="Firstname" class="inpt">
                        </div>
                        <div class="col-lg-4">
                            <input type="text" onkeydown="return alphaOnly(event)" name="middlename" id="mmname" placeholder="Middlename" class="inpt">

                        </div>
                        <div class="col-lg-4">
                            <input type="text" onkeydown="return alphaOnly(event)" name="lastname" id="llname" placeholder="Lastname" class="inpt">
                        </div>
