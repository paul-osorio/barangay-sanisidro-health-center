...
<script>
    $(document).ready(function() {
        ...
        function fetchnotification(start) {
            $.ajax({
                url: '<?= base_url('PatientAuth/fetchNotification') ?>',
                method: "POST",
                data: {
                    start: start,
                },
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            html += '<a href="' + base_url + '/P/ViewNotifications/' + data[i].id + '" class="text-decoration-none text-dark">'
                            ...
                            html += '<i class="' + data[i].icon + ' ico"></i>';
                            html += '</div></div>';

                            html += '<div class="col-lg-11">';
                            html += '<div class="ms-3">';
                            html += '<div class="d-flex justify-content-between"><p class="titlenotif">' + data[i].title + '</p><span class="text-secondary" style="font-size: 12px">' + data[i].time + '</span></div>';
                            html += '<p class="notifcontent">' + data[i].content + '</p>';
                            html += '</div>';
                            html += ' </div>';
                            html += ' </div>';
                            html += '</div></a>';
